...
def vocab_build(vocab_path, corpus_path, min_count):
    ...
    word2id['<UNK>'] = new_id
    word2id['<PAD>'] = 0
    print("由train_data来构造一个(统计非重复字)字典{'第一个字':[对应的id,该字出现的次数],'第二个字':[对应的id,该字出现的次数], , ,}")
    print(len(word2id))
...
def read_dictionary(vocab_path):
    print("通过pickle模块自带的load方法(反序列化方法)加载输出word2id")
    """
...
def random_embedding(vocab, embedding_dim):
    print  ('# 输入vocab，vocab就是前面得到的word2id，embedding_dim=300')
    """
